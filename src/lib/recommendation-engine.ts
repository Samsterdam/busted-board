...
import { fetchMovieBucket, fetchShowBucket } from "./feed-buckets";
...
import { kidsDiscoverParams, KIDS_FETCH_BUDGET_MULTIPLIER, KIDS_BUCKET_PAGES } from "./config/kids";
...
export async function buildFeed(
  ...
  // Kid-safe cert/genre restrictions (empty objects when not in kids mode).
  const kidsMovie = kidsDiscoverParams("movie", kidsMode);
  const kidsTv = kidsDiscoverParams("tv", kidsMode);
  ...
  // Kids mode deepens each bucket (multiple TMDB pages) since its genre+cert
  // pool is small and a large watch history can exhaust page 1 entirely.
  const kidsPages = kidsMode ? KIDS_BUCKET_PAGES : 1;

  // In kids mode the catalog (no genre/cert data) and trending bucket (no
  // discover params to filter on) can't be made kid-safe, so they're dropped.
  const [catalogItems, trendingMovies, hiddenGems, classics, recentMovies, onPlatformMovies, freePlatformMovies,
    ...
    kidsMode ? Promise.resolve([] as CatalogCandidate[]) : queryCatalogCandidates(userPlatformSlugs, region),
    kidsMode
      ? Promise.resolve([] as (TmdbMovie & { media_type: "movie" })[])
      : getTrendingMovies().then((r) => (r.results ?? []).map((m) => ({ ...m, media_type: "movie" as const }))).catch(() => [] as (TmdbMovie & { media_type: "movie" })[]),
    fetchMovieBucket({ "vote_average.gte": "7.5", "vote_count.gte": "500", "popularity.lte": "20", sort_by: "vote_average.desc", ...kidsMovie }, kidsPages),
    fetchMovieBucket({ "vote_average.gte": "7.5", "primary_release_date.lte": `${currentYear - CLASSICS_MIN_AGE_YEARS}-12-31`, sort_by: "vote_average.desc", ...kidsMovie }, kidsPages),
    fetchMovieBucket({ "primary_release_date.gte": recentCutoff.toISOString().split("T")[0], sort_by: "popularity.desc", ...kidsMovie }, kidsPages),
    userPlatformTmdbIds.length > 0 ? fetchMovieBucket({ ...providerParam, sort_by: "vote_average.desc", "vote_count.gte": "50", ...kidsMovie }, kidsPages) : Promise.resolve([] as (TmdbMovie & { media_type: "movie" })[]),
    freePlatformIds.length > 0 ? fetchMovieBucket({ ...freeProviderParam, sort_by: "vote_average.desc", "vote_count.gte": "10", ...kidsMovie }, kidsPages) : Promise.resolve([] as (TmdbMovie & { media_type: "movie" })[]),
    fetchShowBucket({ "vote_average.gte": "7.5", "vote_count.gte": "500", sort_by: "vote_average.desc", ...providerParam, ...kidsTv }, kidsPages),
    fetchShowBucket({ sort_by: "popularity.desc", "vote_count.gte": "100", ...providerParam, ...kidsTv }, kidsPages),
    fetchShowBucket({ "first_air_date.gte": recentCutoff.toISOString().split("T")[0], sort_by: "popularity.desc", ...providerParam, ...kidsTv }, kidsPages),
  ]);
  ...
  // Kids mode drops the catalog + trending and narrows by cert/genre, so widen
  // the candidate budget to keep the on-platform feed from starving.
  const lookupLimit = kidsMode ? FEED_PROVIDER_LOOKUP_LIMIT * KIDS_FETCH_BUDGET_MULTIPLIER : FEED_PROVIDER_LOOKUP_LIMIT;
  const nonCatalogBudget = Math.max(0, lookupLimit - filteredCatalog.length);
  ...
  if (tasteProfile && !kidsMode && rankableCandidates.length > 0) {
    ...
    // Generic ranking (no taste profile, or kids mode bypasses it): order by
    // audience score so broadly-loved titles surface first, not bucket order.
    const ordered = kidsMode
      ? [...rankableCandidates].sort((a, b) => b.item.vote_average - a.item.vote_average)
      : rankableCandidates;
    ranked = ordered.map((c, i) => ({
      tmdb_id: c.item.id,
      rank: i + 1,
      why_youll_like_this: "",
    }));
...
export async function buildMoreFeed(
  ...
  // 6 strategies: alternating movie/TV each page. Kid-safe params are merged per
  // strategy by media type (empty objects when not in kids mode).
  const strategies: Array<{ fn: "movie" | "tv"; params: Record<string, string> }> = [
    { fn: "movie", params: { ...providerParam, sort_by: "vote_average.desc", "vote_count.gte": "500", ...kidsMovie } },
    { fn: "tv",    params: { ...providerParam, sort_by: "vote_average.desc", "vote_count.gte": "500", ...kidsTv } },
    { fn: "movie", params: { ...providerParam, sort_by: "popularity.desc", ...kidsMovie } },
    { fn: "tv",    params: { ...providerParam, sort_by: "popularity.desc", ...kidsTv } },
    { fn: "movie", params: { ...providerParam, sort_by: "vote_average.desc", "vote_count.gte": "200", "primary_release_date.lte": `${new Date().getFullYear() - MORE_FEED_OLD_MIN_AGE_YEARS}-12-31`, ...kidsMovie } },
    { fn: "tv",    params: { ...providerParam, sort_by: "primary_release_date.desc", "vote_average.gte": "6.5", ...kidsTv } },
  ];
  ...
  const [tmdbResults, catalogItems] = await Promise.all([
    ...
    // Catalog has no genre/cert data → can't be made kid-safe; skip in kids mode.
    kidsMode ? Promise.resolve([] as CatalogCandidate[]) : queryCatalogCandidates(userPlatformSlugs, region),
  ]);
  ...
  const moreLookupLimit = kidsMode ? MORE_FEED_PROVIDER_LOOKUP_LIMIT * KIDS_FETCH_BUDGET_MULTIPLIER : MORE_FEED_PROVIDER_LOOKUP_LIMIT;
  const nonCatalogBudget = Math.max(0, moreLookupLimit - filteredCatalog.length);
